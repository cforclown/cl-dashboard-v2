...
import altAvatar from '../../Assets/images/clown.png'
...
function Avatar({ 
    ...
    function getAvatarUrl(avatarData){
        if(!avatarData)
            return null;
        if(!avatarData._id || !avatarData.filename)
            return null;

        return `${process.env.ATTACHMENT_URL}/${avatarData._id}/${avatarData.filename}`
    }
    ...
    return (
        ...
        {
            avatarUrl ?
            <img src={getAvatarUrl(avatarUrl)} onError={(e)=>{e.target.onerror = null; e.target.src=altAvatar}}/>
            :
